...
volatile uint32_t freq_ausgabe = 0;
volatile uint32_tfreq_ausgabe_high1 = 0;
...
ISR(TIMER0_COMPA_vect)
{
    ...
    tick1++;
    // Torzeit: 1s, Sekundenbasis (16MHz/1024/125/125=1Hz)
    if(tick1 == 125) {
...
int main()
{
    ...
    //TO-DO: LCD-Init und Testausgabe bei Systemstart

    sei();  // Interrupts erlauben

    while(1) {
        freq_ausgabe_high1 = (freq1 * korrekturwert);  // Frequenz mit Korrektur berechnen
        //TO-DO: freq_ausgabe_high1 -> Auf LCD ausgeben
    }
}
